...
class WebUiHelperRegressionTests(unittest.TestCase):
    ...
    def test_sms_settings_live_in_accounts_and_expose_log_action(self):
        html = self.client.get("/").get_data(as_text=True)
        self.assertNotIn('id="tab-sms"', html)
        self.assertNotIn('data-tab="sms"', html)
        self.assertIn('id="smsSettingsModal"', html)
        self.assertIn('id="btnSmsSettingsV2"', html)
        ...
        self.assertIn('自动选择（价格/库存优先）', html)
        self.assertIn('id="btnSmsCenterStartV2"', html)
        self.assertIn('id="btnSmsLogV2"', html)
        self.assertIn('供应商 / 价格 / 库存', html)
        self.assertNotIn('sms-tier-table', html)
