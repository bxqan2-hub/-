...
class GPTMailProviderTests(unittest.TestCase):
    def test_parse_sources_keeps_gptmail_in_order(self):
        self.assertEqual(
            email_provider.parse_email_sources("outlook,gptmail,generic_api,domain_api,mailnest,cloudmail"),
            ["outlook", "gptmail", "generic_api", "domain_api", "mailnest", "cloudmail"],
        )
    ...
    @patch("core.inbox_mate_mail_client.pick_account")
    def test_acquire_email_honors_explicit_inbox_mate_source(self, pick_account):
        pick_account.return_value.email = "fresh@mail.com"

        self.assertEqual(email_provider.acquire_email("inbox_mate"), "fresh@mail.com")
    ...
    def test_wait_for_otp_passes_rejected_codes_only_to_generic_api(self, resolve, fetch_latest_otp):
        with patch.object(email_config, "USE_EMAIL_SERVICE", True):
            self.assertEqual(
