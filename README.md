...
- 接码平台仅保留 HeroSMS，使用其 SMS-Activate 兼容 API。
...
ROXY_DEFAULT_OS = "Windows"  # 可选 Windows / macOS
ROXY_CREATE_USE_PROXY_POOL = True
```

也可以在 WebUI 的“配置 → RoxyBrowser → 固定系统”中选择 Windows 或 macOS；选择后会自动关闭随机 OS，并从下一个新建 Profile 开始生效。
...
也可以使用 Cliproxy API 白名单模式。启用后，静态代理池可以留空，每个新的协议/Cloak/Roxy 注册会话都会实时获取一条代理：

```dotenv
PROXY_POOL="[]"
PROXY_API_ENABLED="True"
PROXY_API_URL="https://api.cliproxy.io/white/api?region=US&num=1&time=10&format=n&type=txt"
PROXY_API_PROFILES="https://api.cliproxy.io/white/api?region=US&num=1&time=10&format=n&type=txt\nhttps://api.cliproxy.io/white/api?region=Rand&num=1&format=n&type=txt"
PROXY_API_ACTIVE="US"
PROXY_API_PROTOCOL="socks5h"
PROXY_API_TIMEOUT="20"
PROXY_API_MAX_ATTEMPTS="3"
PROXY_API_RETRY_DELAY="3"
PROXY_API_VALIDATE="True"
PROXY_API_VALIDATE_TIMEOUT="12"
PROXY_API_CACHE_SECONDS="0"
PROXY_API_FAIL_CLOSED="True"
```

- `format=n&type=txt` 返回的 `host:port` 会自动转换成 `socks5h://host:port`。
- `PROXY_API_PROFILES` 每行直接粘贴一个 API URL；系统从 `region=US/JP/Rand` 自动识别并显示“美国/日本/随机地区”。旧的 `地区|URL` 格式仍可兼容读取。
- `PROXY_API_ACTIVE` 指定当前地区。修改并保存后，只影响之后新建的注册会话。
...
接码配置在 WebUI「配置 → 接码平台」或 `config/codex.py`。API 文档见 [HeroSMS API](https://hero-sms.com/cn/api)：

```python
SMS_API_BASE = "https://hero-sms.com/stubs/handler_api.php"
SMS_API_KEY = "你的 HeroSMS API Key"
SMS_SERVICE = "dr"       # OpenAI 服务代码
SMS_COUNTRY = "auto"     # 自动选择价格最低且有库存的国家
SMS_MAX_PRICE = "0.15"   # 单号金额上限，自动选国和最终取号均强制使用
SMS_MAX_RETRIES = 10
SMS_CODE_WAIT = 120
SMS_POLL_INTERVAL = 5
```

程序按 `getNumber → getStatus → setStatus(6/8)` 完成取号、收码、完成或取消。HeroSMS API Key 实际保存于项目根目录 `.env`。
...
| 账号 | 查看账号、复制 token、单个/批量接码授权、批量删除账号 |
...
WebUI 账号页可点击每行「接码授权」，或勾选账号后点击「一键接码所选账号」。系统会重新登录并收取邮箱 OTP，按 `SMS_MAX_PRICE` 自动选择最低价且有库存的 HeroSMS 国家和号码，完成手机绑定与 Codex OAuth，最后提交 CPA 或保存本地 JSON。
...
账号页面保留独立的“检测 OAICS/CSLIVE”能力：每个账号只创建一次最小 custom Plus Checkout，按返回的 `oaics_*`、`checkout_provider=open_ai/oaics` 或 `cs_live_*` 分类后立即停止。检测不附加促销，也不会执行税区、支付方式、ConfirmationToken 或 confirm。设置页分别维护“套餐检测静态代理池”和“Checkout 检测静态代理池”：点击“加入代理池”可一次粘贴多条静态代理，系统直接读取代理用户名中的 `region-PH`、`region-JP` 等两位国家标签并自动归类；只有未携带 `region-XX` 的普通静态代理才探测真实出口。选择国家后，多并发任务在该国家池内随机洗牌轮转取代理。两套检测池互不复用，也不会再调用动态代理 API。
...
`start-webui.bat` 每次启动前都会运行快速环境自检；依赖不完整时会直接
指出缺少的组件，不再等到账号检测过程中才显示模糊的 Sentinel 错误。
