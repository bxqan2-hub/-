...
def _resolve_oauth_continue_url(
    session: BrowserSession,
    validate_result: dict,
    *,
    repair_profile_name: str | None = None,
    repair_profile_birthday: str | None = None,
) -> tuple[str, str]:
    ...
    is_about_you = "about-you" in str(continue_url) or page_type in {"about_you", "about-you"}
    if not is_about_you:
        return str(continue_url), "https://auth.openai.com/email-verification"
    if not repair_profile_name or not repair_profile_birthday:
        raise RuntimeError(
            f"该邮箱登录后进入资料页，疑似不是完整已注册账号: "
            f"page_type={page_type}, continue_url={continue_url}"
        )

    logger.warning("[查活] 注册后 AT 恢复进入资料页，补交资料并继续 OAuth callback")
    about_url = str(continue_url) if "about-you" in str(continue_url) else None
    navigate_about_you(session, about_url)
    sentinel_response = request_sentinel_token(session, "oauth_create_account")
    ...
    create_result = create_account(
        session,
        repair_profile_name,
        repair_profile_birthday,
        sentinel_header,
        so_header,
    )
    repaired_continue_url = create_result.get("continue_url")
    if not repaired_continue_url:
        raise RuntimeError(f"AT 恢复补交资料后没有 OAuth continue_url: {create_result}")
    return str(repaired_continue_url), "https://auth.openai.com/about-you"
...
def check_account_liveness(
    ...
    *,
    ...
    try:
        ...
        continue_url, callback_referer = _resolve_oauth_continue_url(
            session,
            validate_result,
            repair_profile_name=repair_profile_name,
            repair_profile_birthday=repair_profile_birthday,
        )
