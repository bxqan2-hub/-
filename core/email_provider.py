...
EMAIL_SOURCE 支持单个或多个来源：
    ...
    "inbox_mate"        # Inbox Mate 账号/密码任务 API
    "gptmail"
    "mailnest"
    "cloudmail"
...
_VALID_SOURCES = ("outlook", "generic_api", "domain_api", "inbox_mate", "cloudflare_domain", "cloudflare", "gptmail", "mailnest", "cloudmail")
...
def _pick_from_source(source: str) -> str:
    ...
    if source == "domain_api":
        from core.domain_api_mail_client import pick_account
        return pick_account().email
    if source == "inbox_mate":
        from core.inbox_mate_mail_client import pick_account
        return pick_account().email
    if source == "mailnest":
...
def acquire_email(email_source=None) -> str:
    """根据指定来源或全局 EMAIL_SOURCE 领取注册邮箱。"""
    sources = parse_email_sources(email_source)
...
def resolve_email_source(email: str) -> str:
    ...
    if generic_row:
        if str(generic_row.get("provider") or "").strip().lower() == "domain_api":
            return "domain_api"
        if str(generic_row.get("provider") or "").strip().lower() == "inbox_mate":
            return "inbox_mate"
        return "generic_api"
...
def wait_for_otp(
    ...
    if source == "domain_api":
        from core.domain_api_mail_client import fetch_latest_otp
        return fetch_latest_otp(
            email,
            after_ts=after_ts,
            exclude_codes=exclude_codes,
    ...
    *,
    ...
    if source == "outlook":
        ...
        changed = db.release_unconsumed_generic_api_email(
            ...
            failure_limit=max(
                1,
                int(getattr(_email_cfg, "GENERIC_API_REGISTRATION_FAILURE_LIMIT", 2) or 2),
            ),
        )
    elif source == "cloudflare_domain":
