...
手机号验证通过 core.sms_provider 调用 HeroSMS 自动收码。
定义的本地 L 取号服务。
...
def _codex_auth_url_source() -> str:
    source = str(getattr(_cfg, "CODEX_AUTH_URL_SOURCE", "cpa") or "cpa").strip().lower()
    # CPA is optional. If its endpoint/key is incomplete, keep OAuth usable via
    # the built-in local PKCE flow instead of failing the whole authorization.
    if source == "cpa" and not _cpa_management_configured():
        return "local"
    return source


def _cpa_management_configured() -> bool:
...
def _do_phone_verification(session: BrowserSession) -> None:
    ...
    实际平台适配在 core.sms_provider，通过 HeroSMS handler_api.php 完成取号、收码和释放。
    """
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            except (sms_provider.SmsNoBalanceError, sms_provider.SmsProviderConfigurationError):
                # 余额不足或静态配置错误，重试/换号都无意义，直接抛。
                raise
            except sms_provider.SmsNoNumbersError:
                logger.warning("[Codex] 所有符合金额上限的可用地区均已尝试，本轮停止继续取号")
                raise
...
def exchange_codex_token(session: BrowserSession, code: str, code_verifier: str) -> dict:
    ...
    resp = _with_net_retry(
        "exchange token",
        lambda: session.post(_cfg.CODEX_TOKEN_URL, headers=headers, data=urlencode(data)),
    )
...
def run_codex_oauth(
    ...
    try:
        ...
        if isinstance(exc, CodexRetryStopped):
            logger.info("[Codex] 用户停止授权，立即结束当前会话：%s", email)
            raise
