# -*- coding: utf-8 -*-
"""Background UPI qualification detection for account rows.

The detector creates one IN/INR Checkout, initializes Stripe once,
records whether ``upi`` is published, and stops before every payment action.
"""
...
_EXECUTOR = ThreadPoolExecutor(max_workers=DEFAULT_WORKERS, thread_name_prefix="upi-check")
...
def get_executor(max_workers: int | None = None) -> ThreadPoolExecutor:
    ...
    with _EXECUTOR_LOCK:
        if requested != _EXECUTOR_WORKERS:
            ...
            _EXECUTOR = ThreadPoolExecutor(max_workers=requested, thread_name_prefix="upi-check")
        return _EXECUTOR
...
def check_upi(access_token: str, *, proxy: str | None = None, generation: int | None = None) -> dict:
    if not proxy:
        return {"ok": False, "upi": False, "error": "UPI 查询需要印度代理", "confirm_sent": False}
    operation_generation = account_operation_control.snapshot() if generation is None else int(generation)
    cancel_check = lambda: account_operation_control.raise_if_cancelled(operation_generation)
    result: dict = {"ok": False, "upi": False}
    status_code = 502
    used_transport = ""
    attempts = 0
    for candidate in _transport_candidates(proxy):
        ...
        try:
            result, status_code = get_pay153_module().detect_upi(body, check_cancelled=cancel_check)
        except account_operation_control.AccountOperationStopped:
            raise
        except Exception as exc:
            result = {"ok": False, "upi": False, "error": f"UPI 检测执行异常：{type(exc).__name__}"}
            status_code = 502
        if not isinstance(result, dict):
            result = {"ok": False, "upi": False, "error": "PAY.153 返回格式不正确"}
    ...
    result.setdefault("upi", False)
    result.setdefault("confirm_sent", False)
    if status_code >= 400:
        result["ok"] = False
        result.setdefault("error", f"PAY.153 UPI 检测失败（状态 {status_code}）")
    return result


def _retryable(result: dict) -> bool:
    if result.get("ok") or result.get("upi"):
        return False
...
def _run_with_proxy_retry(
    *, account_id: int, access_token: str, proxies: list[str],
    ...
    operation_generation = account_operation_control.snapshot() if generation is None else int(generation)
    if not db.mark_account_upi_running(account_id, generation=operation_generation):
        return {"ok": False, "upi": False, "error": "账号已删除或检测状态已重置"}
    ...
    attempted: list[str] = []
    result: dict = {"ok": False, "upi": False, "error": "没有可用的 IN 代理"}
    try:
        for index, proxy in enumerate(pool[:total]):
            account_operation_control.raise_if_cancelled(operation_generation)
            if deadline is not None and time.monotonic() >= deadline:
                result["error"] = f"UPI 检测总耗时超过 {int(total_timeout or 0)} 秒"
                break
            result = check_upi(access_token, proxy=proxy, generation=operation_generation)
            account_operation_control.raise_if_cancelled(operation_generation)
            attempted.append(f"proxy-{index + 1}")
    ...
    if not result.get("stopped"):
        db.update_account_upi(account_id, result, generation=operation_generation)
    return result
...
def enqueue(
    *, account_id: int, access_token: str, proxies: list[str], trigger: str = "manual",
    ...
    if not candidates:
        return {"accepted": False, "busy": False, "error": "UPI 检测没有可用的 IN 代理"}
    if not _QUEUE_SLOTS.acquire(blocking=False):
        return {"accepted": False, "busy": False, "error": "UPI 检测队列已满"}
    generation = account_operation_control.snapshot()
    try:
        claimed = db.claim_account_upi(account_id, trigger=trigger, generation=generation)
    except Exception:
        _QUEUE_SLOTS.release()
        raise
    if not claimed:
        _QUEUE_SLOTS.release()
        return {"accepted": False, "busy": True, "error": "该账号正在检测 UPI 资格"}
    try:
        ...
        _QUEUE_SLOTS.release()
        result = {"ok": False, "upi": False, "error": f"UPI 检测任务入队失败：{type(exc).__name__}"}
        db.update_account_upi(account_id, result, generation=generation)
        return {"accepted": False, "busy": False, "error": result["error"]}
    return {"accepted": True, "busy": False, "account_id": account_id, "status": "queued"}


__all__ = ["DEFAULT_WORKERS", "MAX_WORKERS", "MAX_PROXY_RETRIES", "check_upi", "enqueue", "get_executor"]
