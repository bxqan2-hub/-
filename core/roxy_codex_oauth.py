...
def _fill_email_and_otp(driver, email: str, otp_provider, auth_url: str) -> None:
    ...
    def _resend_email_otp(reason: str) -> bool:
        ...
        otp_after_ts = time.time()
        try:
            result = _click_resend_email_otp(driver, timeout=25)
            if result.get("advanced"):
                logger.info("[Codex][Browser] 重发前页面已进入下一步：%s", result["advanced"])
                return True
            logger.info(
                "[Codex][Browser] 已在当前邮箱验证页直接点击重新发送，接下来最多等待 %ss 新验证码",
                otp_wait_seconds,
            )
            human_delay("api")
            return False
            ...
            logger.warning(
                "[Codex][Browser] 当前页直接重发失败，回退重开授权页：%s: %s",
                type(exc).__name__,
                str(exc)[:180],
            )
    ...
    for otp_attempt in range(1, max_otp_attempts + 1):
        ...
        try:
            ...
            if otp_attempt >= max_otp_attempts:
                raise
            logger.warning(
                "[Codex][Browser] 本轮 %ss 内未收到新邮箱 OTP，直接点击当前页重新发送后继续等待（下一轮 %s/%s）：%s: %s",
                otp_wait_seconds,
                otp_attempt + 1,
                max_otp_attempts,
                type(exc).__name__,
            ...
            if _resend_email_otp("当前页重发失败后的兜底"):
                return
            continue
        ...
        logger.warning(
            "[Codex][Browser] 邮箱验证码错误/过期，保留当前验证页并直接重发新码（%s/%s）",
            otp_attempt + 1,
            max_otp_attempts,
        )
        if _resend_email_otp("当前页重发失败后的兜底"):
...
def _wait_for_fresh_email_otp(otp_provider, email: str, after_ts: float, used_codes: set[str] | None = None, timeout: int = 90) -> str:
    ...
    try:
        ...
        if "unexpected keyword" not in str(exc).lower():
            raise
        code = otp_provider(email, after_ts=after_ts)
...
def _select_sms_channel_or_raise(driver) -> None:
    ...
    const labelText = el => {
      const direct = el.id ? document.querySelector(`label[for="${CSS.escape(el.id)}"]`) : null;
      const parent = el.closest('label');
      return `${direct?.innerText || ''} ${parent?.innerText || ''} ${el.getAttribute('aria-label') || ''}`.trim();
    };
    const isSms = el => /^(sms|text|text_message|text-message)$/i.test(el.value || '')
      || /\bsms\b|text message|短信|簡訊|ショートメッセージ/i.test(labelText(el));
    const isWhatsapp = el => /whats\s*app/i.test(`${el.value || ''} ${labelText(el)}`);
    const sms = radios.find(isSms);
    const whatsapp = radios.find(isWhatsapp);
    if (!sms) return {ok: !whatsapp, reason: whatsapp ? 'whatsapp_only' : 'default_sms', radios: radios.map(el => ({value:el.value,checked:el.checked,label:labelText(el)}))};
    const label = sms.id ? document.querySelector(`label[for="${CSS.escape(sms.id)}"]`) : sms.closest('label');
    ...
    return {
      ok: !!sms.checked && !(whatsapp && whatsapp.checked),
      reason: sms.checked ? ((whatsapp && whatsapp.checked) ? 'whatsapp_still_checked' : 'sms_checked') : 'sms_not_checked',
      sms: {value:sms.value,checked:sms.checked,label:labelText(sms)},
      whatsapp: whatsapp ? {value:whatsapp.value,checked:whatsapp.checked,label:labelText(whatsapp)} : null,
    };
    """) or {}
    if not selected.get("ok"):
        raise RuntimeError(f"sms_channel_not_selected: {selected}")
    logger.info("[Codex][Browser] SMS 通道已确认：%s", selected)


def _phone_code_uses_whatsapp(state: dict) -> bool:
    body = str(state.get("bodyText") or "").casefold()
    patterns = (
        r"(?:code|verification).{0,45}(?:sent|send).{0,30}whats\s*app",
        r"(?:sent|send).{0,45}(?:code|verification).{0,30}whats\s*app",
        r"whats\s*app.{0,45}(?:code|verification|sent)",
        r"验证码.{0,30}(?:发送|送到).{0,20}whats\s*app",
        r"whats\s*app.{0,30}验证码",
        r"whats\s*app.{0,30}(?:コード|送信)",
    )
...
def _set_phone_value(driver, phone: str, *, timeout: int = 10) -> dict:
    ...
    # 国家选择器未及时渲染时，React-Aria 可能在可见框保留本地 trunk 0
    # （例如 +44 073...），但隐藏 phoneNumber 已经是准确的 E.164。只要隐藏
    # 字段与请求号码一致且可见框非空，交给页面 DOM 提交，不要过早换号。
    hidden_matches = bool(hidden_digits) and hidden_digits == e164_digits
...
def _verify_add_phone_value_before_submit(driver, expected_e164: str) -> dict:
    ...
    const hiddenMatches = !!hidden && hiddenDigits === expectedDigits;
    const ok = !!visibleDigits && (visibleDigits === expectedDigits || hiddenMatches)
      && (!hidden || hiddenMatches);
    return {ok, visibleValue, hiddenValue, expected, visibleDigits, hiddenDigits, expectedDigits, hiddenMatches, url: location.href};
    """, expected_e164)
    if not result or not result.get("ok"):
...
def _wait_after_phone_send(driver, timeout: int = 120) -> str:
    end = time.time() + timeout
    last = {}
    ...
    while time.time() < end:
        ...
        if _is_phone_code_state(last):
            if _phone_code_uses_whatsapp(last):
                raise RuntimeError(f"whatsapp_delivery: {(last.get('bodyText') or '')[:240]}")
            return 'code_page'
        # add-phone 仍存在时只依据 DOM 状态继续等待，避免页面文案尚未更新
        # 就被误判为发送失败并立即换号。
...
def _classify_phone_page_failure(state: dict) -> str:
    if _is_phone_code_state(state):
        return ''
    # 只有页面完全没有 SMS 选项时才判定为 WhatsApp-only。提交失败后页面可能重置为
    # 默认勾选 WhatsApp；如果 SMS 仍可选，把它报成 whatsapp_channel 会掩盖真实失败原因。
    radios = state.get('radios') or []
    radio_values = [str(r.get('value', '')).lower().replace(' ', '') for r in radios]
    has_whatsapp = any('whatsapp' in value for value in radio_values)
    has_sms = any(value in ('sms', 'text', 'text_message', 'text-message') for value in radio_values)
    if has_whatsapp and not has_sms:
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                logger.info("[Codex][Browser] 提交前二次确认 SMS 通道，防止页面重渲染回 WhatsApp")
                _select_sms_channel_or_raise(driver)
                ...
                logger.info(
                    "[Codex][Browser] 通过 DOM 等待 phone-verification 跳转，最长等待 %ss",
                    phone_page_wait,
                )
                ...
                if isinstance(exc, CodexRetryStopped):
                    ...
                    logger.info("[Codex][Browser] 收到停止信号，终止换号并进入窗口清理")
                    raise
                last_err = exc
                ...
                if activation_country:
                    ...
                    logger.info(
                        "[Codex][Browser] 国家 id=%s 本次失败，后续尝试将自动换其它地区",
                        activation_country,
                    )
                if email:
                    from core.codex_retry_service import check_stop_requested
...
def _run_roxy_codex_oauth_once(
    ...
    client = None if reuse_existing_profile else RoxyBrowserClient(profile_proxy=proxy)
    from config import codex as codex_cfg
    codex_headless = bool(getattr(codex_cfg, "CODEX_HEADLESS", True))
    opened = existing_opened if reuse_existing_profile else client.open_profile(headless=codex_headless)
    ...
    try:
        ...
        logger.info("[Codex][Browser] 用户停止授权，立即关闭窗口并清理环境：%s", email)
        raise
