...
class CloakElement:
    ...
    def send_keys(self, *values: str) -> None:
        """兼容 Selenium send_keys：向当前输入框追加字符。

        注意：不能用 Playwright fill() 逐字输入，fill 会整框覆盖，
        导致 human type 逐字符时最终只剩最后一个字母。
        """
        ...
        try:
            if self.locator is not None:
                try:
                    self.locator.focus(timeout=2000)
                except Exception:
                    self.click()
            else:
                self.click()
        ...
        # Selenium Keys.CONTROL/COMMAND + 'a' => 全选
        if "" in text or "" in text or "" in text or "command" in lower or "control" in lower:
            try:
            ...
            if rest_l in ("", "a", "command", "control"):
                return
            text = rest
            lower = text.lower()

        # Backspace / Delete
        if text in ("",) or lower == "backspace":
            self.page.keyboard.press("Backspace")
            return
        if text in ("",) or lower == "delete":
        ...
        # 追加输入（Selenium 语义）
        try:
            self.page.keyboard.type(cleaned, delay=18)
            return
        except Exception:
            pass
        ...
        try:
            if self.locator is not None:
                current = self.locator.input_value(timeout=1000) or ""
            elif self.handle is not None:
                current = self.handle.input_value(timeout=1000) or ""
        except Exception:
            current = ""
        final = f"{current}{cleaned}"
        try:
            if self.locator is not None:
                self.locator.fill(final, timeout=10000)
            else:
                self.handle.fill(final, timeout=10000)
        except Exception:
            self.page.keyboard.type(cleaned, delay=25)
