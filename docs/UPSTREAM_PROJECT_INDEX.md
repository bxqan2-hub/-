...
- 后续真实批次 `a08bbd5c7e23` 使用目标 6、并发 2、等待 4–8 秒完成 3 轮；本轮只优化注册页控件和倒计时卡片显示，调度与注册执行链路保持原实现。
...
- 旧批次 740/751 各一次 5 秒邮箱列表/详情超时即提前失败，邮件分别早于结束 14/21 秒到达且 parser 可识别。本地 yangyang 路线在既有总预算内补一次短重试，仅瞬态错误触发，保留列表/详情阶段、取消、时间与旧码过滤；不另建 provider 或新增配置。
...
- 第二轮真实复测 767–776 使用上述新配置、HEAD `142b39c`、端口 5002：6/10 完成注册/密码/MFA/只读 Token，2 个补密码邮箱等待耗尽，1 个 OTP 提交无接受信号，1 个出口漂移。9/10 实际进入邮箱 API；771、773 的瞬态错误各经一次短重试后成功，验证重试修复有效。结果与失败分类见续查报告，不据此放宽 OTP/出口门禁。
...
- 固定窗口包含辅助结束后至少 60 秒。1024proxy 后续余额 10.09 → 10.06 GB，名义约 30 MB，与主端点 29.25 MB 观测吻合；Cliproxy 4.74 → 4.723 GB 当前名义约 17 MB，尚非结算终值。用户明确接受两轮结果并要求交付，不再等待或启动新任务。含另列 SG 的整轮观测分别 29.354703 / 26.780208 MB，均低于 40 MB；截图精度、Cliproxy 未完全刷新及短连接采样边界保留，不声称字节级账单闭环。
- 交付前本地缓存/代理/默认值回归 116 passed、270 subtests passed，保留既有依赖 warning；根因、简短修复说明、最终验收及逐账号表见 `docs/2026-09-14_Roxy十账号流量根因复测-report.md`。1024proxy Plan 查询 9/10（723 为独立 SG TLS 错误），Cliproxy Plan 10/10；两轮注册/密码/2FA 全部成功。账号邮箱报告与原始证据在忽略目录。
...
- 空 Profile 的实际 argv 已逐项确认全部 7 个原效率参数存在；100 秒驻留 NetLog 组件下载/更新检查均为 0，供应商连接采样约 1.23 MB（不是完整账号成本）。当前暂停新增流量，待用户明确恢复并确定同窗口计量依据后，再进行独立十账号对账；结果继续记录在 `docs/2026-09-14_Roxy十账号流量根因复测-report.md`。
- 用户确认套餐端点为 `us.arxlabs.io:3010`；余额依次为 4.930 → 4.899 → 4.875 → 4.865 → 4.843 GB，最新名义减少 87 MB，尚未确认最终结算。独立复核排除了所观察连接的重复累计，但传输与同一套餐扣量仍缺完整对应。Mihomo 全局、端点连接、CDP payload 和供应商余额分别记录。
- 用户要求等待期间不产生新增流量；只做本地证据整理，暂停注册、网络验证及推送。14:52–14:53 us 端点采样为 0，14:54:06 us/sg 单次快照均为 0；不把这些有限窗口推广为所有设备零用量。历史 `proxy_used` 未记录实际 `client.profile_proxy` 的缺口已在复测报告中标明，未在等待期变更业务。
...
- 注册流量列表同时显示新增网络下载与含缓存回放的逻辑总量；历史日志约 2.5–3.3 MiB 是当时的 CDP 页面下载观测，未覆盖全部浏览器后台/协议/其他流程，也未核实为供应商扣量，逻辑资源总量约 38–87 MiB，二者未混算。
