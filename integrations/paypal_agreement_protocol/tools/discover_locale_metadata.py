...
def proxy_for(cc):
 ...
 template=os.environ.get('PAYPAL_DISCOVERY_PROXY_TEMPLATE','').strip()
 if not template:
  raise RuntimeError('PAYPAL_DISCOVERY_PROXY_TEMPLATE is required')
 return template.format(country=cc,sid=sid)
