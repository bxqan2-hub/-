...
def _run_vm_bundle_via_node(chat_req: dict, xor_key: str, flow: str = "oauth_create_account") -> Optional[dict]:
    ...
    try:
        ...
        # 避免把缺少 jsdom / Node 等部署问题误报成普通的 t proof 失败。
        ...
        if marker in output:
            ...
            detail = (result.stderr or result.stdout or "Node helper returned no JSON output").strip()
            detail = " ".join(detail.split())[:600]
            print(f"    [SENTINEL] Node.js helper failed: {detail}")
            return {"_error": f"Node.js Sentinel helper failed: {detail}"}
    except subprocess.TimeoutExpired:
        print("    [SENTINEL] Node.js VM timeout")
        return {"_error": "Node.js Sentinel helper timed out after 30 seconds"}
    except Exception as e:
        print(f"    [SENTINEL] Node.js VM error: {e}")
        return {"_error": f"Node.js Sentinel helper error: {type(e).__name__}: {e}"}
    finally:
        try:
...
class SentinelTokenProvider:
    ...
    async def get_token_pair(self, flow: str, device_id: str) -> tuple[Optional[dict], Optional[dict], dict]:
        ...
        try:
            ...
            vm_data = vm_data or {}
            diagnostics["vm_error"] = str(vm_data.get("_error") or "")
