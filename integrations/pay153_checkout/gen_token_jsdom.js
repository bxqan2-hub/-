...
if (process.argv.includes("--self-check")) {
  const checkSdkPath = process.env.SENTINEL_SDK_PATH || path.join(__dirname, "sentinel_sdk_full.js");
  fs.accessSync(checkSdkPath, fs.constants.R_OK);
  console.log(JSON.stringify({ ok: true, jsdom: require("jsdom/package.json").version, sdk: path.basename(checkSdkPath) }));
  process.exit(0);
}
