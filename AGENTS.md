...
- Roxy 批量注册时每个账号必须创建独立 Profile 并请求独立随机指纹；代理先随机抽取候选，再做真实出口 IP 预检和窗口内复核。按维护者 2026-09-13 的要求，允许并发及连续账号使用同一出口 IP，不再按重复地址或冷却状态停止注册。出口漂移或复核失败仍终止该账号并单独记录；多个 CLI/WebUI 进程的串行运行要求保持不变。
