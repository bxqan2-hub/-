...
#   "domain_api"        — 域名 API 邮箱（自动识别邮箱域名与取码站点）
#   "inbox_mate"        — Inbox Mate 邮箱任务 API（账号、密码自动识别供应商）
...
# 通用取码页的单请求快速失败预算。主请求失败后会立即短重试一次；
# 若连续整轮都不可达，直接结束该邮箱的 OTP 等待，不再空转几分钟。
GENERIC_API_REQUEST_TIMEOUT = 8
GENERIC_API_RETRY_TIMEOUT = 5
GENERIC_API_MAX_CONSECUTIVE_ERRORS = 1
# 同一取码邮箱连续注册失败达到该次数后自动停用，避免反复空等 OTP。
GENERIC_API_REGISTRATION_FAILURE_LIMIT = 2
...
# 可选的默认邮件详情入口。导入账户页面 URL 时会自动从页面站点推导；
# 仅粘贴“邮箱----密码”时可在这里指定，例如 https://mail.example.com/m.php。
DOMAIN_API_BASE = ""

# Inbox Mate 服务地址；可在密码/账号粘贴导入后由任务 API 取码。
INBOX_MATE_BASE = "https://mail.ap1x.xyz"
